...
- Fixed persistent "index saved" notification by correcting `FilePicker.upload` argument order for Foundry V13 (`notify: false` was being passed as `body` instead of `options`).

## [2.1.2] - 2026-02-13

### Fixed

- Added missing `data` directory creation logic to prevent save errors.
- Added `.gitkeep` to `data` directory to ensure it is tracked.
